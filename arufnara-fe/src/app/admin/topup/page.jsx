...
export default function AdminTopUp() {
  ...
  const fetchTopups = async () => {
    try {
      ...
      setError("Gagal mengambil data paket diamond");
      setTopups([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      let responseData;
      if (modalMode === 'create') {
        responseData = await createTopup(formData);
        setTopups(prev => [...prev, responseData]);
      } else {
        const topupId = currentTopup.id || currentTopup._id;
        if (!topupId) throw new Error('ID topup tidak ditemukan');
        responseData = await updateTopup(topupId, formData);
        setTopups(prev => prev.map(topup =>
          (topup.id || topup._id) === topupId
            ? { ...topup, ...responseData }
            : topup
        ));
      }
      ...
      setError(`Gagal ${modalMode === 'create' ? 'membuat' : 'mengupdate'} paket diamond`);
    } finally {
  ...
  const handleDelete = async (topup) => {
    ...
    if (window.confirm("Apakah Anda yakin ingin menghapus paket ini?")) {
      try {
        ...
        setError('Gagal menghapus paket diamond');
      }
    }
  };
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-blue-900 border-b border-blue-100 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div className="flex items-center justify-center w-8 h-8 text-base font-bold text-white rounded-full shadow bg-gradient-to-tr from-blue-300 to-blue-500">
                      ...
                      <td className="px-6 py-4 border-b border-blue-100 whitespace-nowrap">
                        <span className="inline-block px-2 py-1 text-xs font-semibold text-blue-800 border border-blue-100 rounded-lg bg-blue-50">
                          {totalDiamond.toLocaleString()} Total
                        </span>
                      </td>
                      <td className="px-6 py-4 space-x-2 text-sm border-b border-blue-100 whitespace-nowrap">
                        <button
